...
ForgeDesk v1.0.0 is a setup-first local autopilot CLI with explicit auto profiles, optional
repository git hooks, foreground watch mode, ignition folder-open tasks, editor
shortcuts, opt-in test button discovery, local episode status, safe local repair, quiet watch output, `.forgedesk/NOW.md`, local open-file exits, AI-friendly context generation, configurable risk rules, diff-aware content analysis, project-level review templates, and an optional CI evidence gate for evidence-first AI-assisted code changes.

## v1.0: Stable API

All `--json` output schemas are frozen under semver. See `docs/api-contract.md`
for full stability commitments.

Completed features:

- Setup-first local autopilot (`forgedesk setup`, `forgedesk next`)
- Evidence generation and readiness checks
- AI-friendly context generation (`forgedesk context`)
- Deterministic Evidence Score (0-7)
- Configurable risk rules via `.forgedesk/rules.json`
- Diff-aware content risk rules
- pnpm workspace test discovery
- Evidence-as-Code (JSON Schema, validation, preset rules)
- Project-level review templates with variable substitution
- CI evidence gate (`forgedesk ci check`, `forgedesk ci validate`)
- Local editor shortcuts, ignition, watch, hooks
...
- Open existing local NOW, evidence, export, review-context, and PR body
  targets with the system opener.
